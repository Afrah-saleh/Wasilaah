...
struct OptionViewChangable: View {
    ...
    var body: some View {
        HStack {
            ...
            Spacer()
            DropDownMenuChangable(selectedOption: $selectedMonthIndex)
        }
        .padding()
        .onAppear {
            fetchExpenses()
...
struct ChangeableExpenses: View {
    ...
    var body: some View {
        VStack {
            OptionViewChangable(expenses: $expenses, cardViewModel: cardViewModel, chartviewModel: ChartViewViewModel(cardID: ""), selectedMonthIndex: $selectedMonthIndex, currentYear: $currentYear)
                .onChange(of: selectedMonthIndex) { newValue in
